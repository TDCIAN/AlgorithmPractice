...
- Disjoint Set이란
    ...
    - union-by-rank 기법
        ...
        - 초기화시, 모든 원소는 높이(rank)가 0인 개별 집합인 상태에서, 하나씩 원소를 합칠 때,
          union-by-rank 기법을 사용한다면
    ...
    - path compression 기법
        ...
        - union-by-rank와 path compression 기법 사용시 시간 복잡도는 다음 계산식을 만족함이 증명되었음
            ...
            - log*N은 다음 값을 가짐이 증명되었음
                - N이 2^65536 값을 가지더라도, log*N의 값이 5의 값을 가지므로, 
                  union-by-rank와 path compression 기법의 시간 복잡도는 거의 O(1), 즉 상수값에 가깝다고 볼 수 있다.
