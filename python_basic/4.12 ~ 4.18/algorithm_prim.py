...
- Kruskal's algorithm과 Prim's algorithm 비교
    ...
    - Prim's algorithm은 특정 정점에서 시작, 해당 정점에 연결된 가장 가중치가 작은 간선을 선택,
      간선으로 연결된 정점들에 연결된 간선 중에서 가장 가중치가 작은 간선을 택하는 방식으로 MST를 구함
...
heapq.heapify(graph_data) # heapify를 활용하면 저절로 최소 힙 구조를 갖추게 된다

for index in range(len(graph_data)):
    print(heapq.heappop(graph_data))

# print("이제 다 빠졌니: ",queue)

# 참고: collections 라이브러리의 defaultdict 함수 활용하기
# - defaultdict 함수를 사용해서, key에 대한 value를 지정하지 않았을 시, 빈 리스트로 초기화하기
from collections import defaultdict

# list_dict = defaultdict(list) 출력 -> []
list_dict = defaultdict(set) # 출력 -> set()
list_dict['key1']
      ...
      해당 간선 정보를 '최소 신장 트리(mst)'에 삽입
        - 해당 간선에 연결된 인접 정점의 간선들 중, '연결된 노드 집합(connected_nodes)'에 없는 노드와
          연결된 간선들만 간선 리스트(candidate_edge_list)에 삽입
            - '연결된 노드 집합(connected_noded)'에 있는 노드와 연결된 간선들을 간선 리스트에 삽입해도, 
              해당 간선은 스킵될 것이기 때문임
            - 어차피 스킵될 간선을 간선 리스트(candidate_edge_list)에 넣지 않으므로,
...
print("프림: ", prim('A', myedges))
# 프림:  [(5, 'A', 'D'), (6, 'D', 'F'), (7, 'A', 'B'), (7, 'B', 'E'), (5, 'E', 'C'), (9, 'E', 'G')]
...
- 간선이 아닌 노드를 중심으로 우선순위 큐를 적용하는 방식
    ...
    - 가장 key값이 적은 정점: key를 추출한 후(pop하므로 해당 정점: key 정보는 우선순위 큐에서 삭제됨),
...
- 개선된 프림 알고리즘 구현시 고려 사항
    - 우선순위 큐(최소힙) 구조에서, 이미 들어가 있는 데이터의 값 변경시, 최소값을 가지는 데이터를 루트노드로 올려놓도록
      재구성하는 기능이 필요함 
